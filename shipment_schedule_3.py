...
What's new:
1. Added comments for the changed schedules
2. Changed 'def color_font()' to 'def color_font_comment()'
...
def write_data(sheets_value, new_tar_file, cur_tar_file):
    ...
    if cur_tar_file:
        ...
        for row_value in sheets_value:
            ...
            color_font_comment(ws, ship_dict)
    else:
        print('Starting writing data to the target file...')
        num = 0    
        for row_value in sheets_value:
...
def color_font_comment(ws, ship_dict):
    for cell in list(ws.columns)[2]:
        if '*' in cell.value:
            cell.font = Font(color=BLUE)
        elif '!' in cell.value:
            cell.font = Font(color=RED)
            cmt_value1 = ship_dict[cell.value.strip('!')][0]
            cmt_value2 = ship_dict[cell.value.strip('!')][1]
